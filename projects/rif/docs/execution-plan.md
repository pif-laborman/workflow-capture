...
CREATE TABLE tenants (
  ...
  name TEXT NOT NULL,              -- "Rif", "Zoe", etc.
...
ASSISTANT_NAME="Rif"                     # Display name
...
- [ ] Google SSO login works — sees Rif branding, not Pif
...
"Say hello to Rif." Girlfriend starts using it. Pavol monitors for the first few days.
...
**Goal**: Turn Rif into a product others can pay for.
...
`rif.app` or `meetpif.com/rif` — product landing page. "Your AI chief of staff you actually own."
...
| RLS misconfiguration leaks data between tenants | High | Defense-in-depth: API filters by tenant_id in application code AND RLS enforces at DB level. Test with two tenants before deploying Rif. Audit RLS policies in 0C.3. |
...
| Script path refactoring introduces bugs | Medium | Test with throwaway instance (0C.6) before deploying Rif. |
...
*This document is the source of truth for Rif execution. Update it as work progresses. Don't let it drift from reality.*
